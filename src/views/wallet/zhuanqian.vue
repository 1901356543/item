<template>
    <section class="zhuanqian">
        ...
        <div class="cointype" @click="toclose">
        ...
        <div class="cointype">
            <div class="l active-text">{{$t('svg28')}}</div>
        </div>
    ...
    export default {
        ...
        methods: {
            ...
            submit(){
                ...
                if(this.unitCoin(coin) == this.$t(`title`)){
                    if(this.decimal(Number(num)).add(this.zzFee).toFixed(8) * 1 > coinBalances * 1){
                        this.$toast.show(this.$t('lang87'));
                        return;
                    }
                }else{
                    if(Number(num) > coinBalances * 1){
                        this.$toast.show(this.$t('lang87'));
                        return;
                    }
                    if(this.balancesXRP.value * 1 < this.zzFee * 1){
                ...
                if(this.span){
                    ...
                    if(address.length != 42 && address.length != 34){
                ...
                this.account.verifyPassword(this.password).then(async () => {
                    ...
                    this.rcp.preparePayments(option, {
                        ...
                        if(data.engine_result_code == 0){
                            this.updateAddress(option.address, data);
            ...
            updateAddress (address, data){
                this.updateBindding(this.rcp.address, address, data.tx_json.hash);
                return;
                this.rcp.getBalances(address).then(data => {

                }).catch((e) => {
                    if(/Account not found/gi.test(e.message)){
                        console.log(e.message);
                        this.updateBindding(this.rcp.address, address, data.tx_json.hash);
                    }
                });
